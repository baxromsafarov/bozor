// Package config собирает конфигурацию Reviews-сервиса из окружения.
package config
...
const dbName = "bozor_reviews"

const defaultAddr = ":8080"

// Config — конфигурация Reviews-сервиса.
type Config struct {
	Addr     string
	Env      string
	LogLevel string
	...
	// NATSURL — адрес NATS JetStream (публикация bozor.review.created).
	NATSURL string
	// ListingInternalURL — базовый URL внутреннего read-эндпоинта Listing
	// (по объявлению определяется продавец — адресат отзыва).
...
func Load() (*Config, error) {
	...
	return &Config{
		Addr:     config.String("REVIEWS_ADDR", defaultAddr),
...
func Addr() string {
	return config.String("REVIEWS_ADDR", defaultAddr)
}

// ListingTimeout — таймаут чтения объявления из Listing.
const ListingTimeout = 5 * time.Second

// dsn собирает строку подключения к базе сервиса (bozor_reviews).
