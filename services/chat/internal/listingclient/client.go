...
// (/internal/ads/{id}). По объявлению чат определяет продавца (владельца) —
// источник истины, как у модерации/индексатора (fetch-current-state).
...
// GetAd возвращает объявление по id. found=false, если объявления нет
// (404 — удалено), тогда начать диалог нельзя.
func (c *Client) GetAd(ctx context.Context, id string) (domain.AdView, bool, error) {
	...
	case http.StatusOK:
		...
		return domain.AdView{ID: dto.ID, UserID: dto.UserID, Title: dto.Title, Status: dto.Status}, true, nil
