// Command server — точка входа User/Profile-сервиса Bozor.
package main
...
func run() error {
	...
	// NATS JetStream + relay: события bozor.user.updated из outbox в шину.
	...
	profileRepo := repo.NewRepo(pool)

	// Консьюмер bozor.user.created: создание профиля по умолчанию (idempotent,
	// inbox). До 3 попыток, затем DLQ (natsx).
	users := worker.NewUsers(profileRepo, log)
	cc, err := natsx.Consume(ctx, js, events.StreamName, worker.UsersConsumer,
		[]string{events.SubjectUserCreated}, 3, users.Handle)
	if err != nil {
	...
	log.Info("user-profile стартует", slog.String("addr", cfg.Addr), slog.String("env", cfg.Env))
