// Command server — точка входа Location-сервиса Bozor.
package main
...
import (
	...
	"bozor/services/location/internal/app"
	"bozor/services/location/internal/config"
	"bozor/services/location/internal/repo"
	"bozor/services/location/internal/transport"
...
func main() {
	...
	if err := run(); err != nil {
		fmt.Fprintln(os.Stderr, "location:", err)
		os.Exit(1)
	}
}
...
func run() error {
	...
	// Миграции (схема + сид) применяются при старте по прямому DSN (ADR-013).
	...
	svc := app.NewService(repo.NewRepo(pool))
	...
	log.Info("location стартует", slog.String("addr", cfg.Addr), slog.String("env", cfg.Env))
